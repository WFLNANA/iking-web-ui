...
import type { IRange, editor as MEditor, languages } from 'monaco-editor'
import type * as monacoEditor from 'monaco-editor/esm/vs/editor/editor.api'

export type Monaco = typeof monacoEditor

export type languageType = 'plaintext' | 'html' | 'javascript' | 'json' | 'sql' | 'xml' | 'java' | 'typescript'
...
export const handleInputCode = (languageId: languageType, code: string | any[] | object) => {
  ...
  if (paramType.isObject(val) || paramType.isArray(val)) { val = JSON.stringify(val, null, 2) }

  return paramType.isString(val) ? val : `${val}`
}
